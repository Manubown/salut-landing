 ...
 * scrub stages pass 1).
 */
export type UiMode = 'track' | 'install';
...
export class UiPainter {
  ...
  private last = { mode: '' as string, p: -1, open: -1 };
  ...
  draw(mode: UiMode, p: number, open = 1, accent = '#ff2e63'): boolean {
    const l = this.last;
    if (l.mode === mode && Math.abs(l.p - p) < 0.0025 && Math.abs(l.open - open) < 0.0025) {
      return false;
    }
    this.last = { mode, p, open };
